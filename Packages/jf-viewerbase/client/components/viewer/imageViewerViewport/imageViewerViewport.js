...
const loadDisplaySetIntoViewport = (data, templateData) => {
    ...
    if (stack.isClip && stack.frameRate > 0) {
        ...
        JF.viewer.cine.framesPerSecond = parseFloat(stack.frameRate);
    ...
    imagePromise.then(image => {
        ...
        if (multiframeMetadata && multiframeMetadata.averageFrameRate > 0) {
            ...
            fps = JF.viewer.cine.framesPerSecond;
        }

        const cineToolData = {
            loop: JF.viewer.cine.loop,
            framesPerSecond: fps
        };
        ...
        const onImageRendered = (event) => {
            ...
            // Save the current viewport into the JF.viewer.data global variable
            const viewport = cornerstone.getViewport(element);
            layoutManager.viewportData[viewportIndex].viewport = viewport;
            JF.viewer.data.loadedSeriesData[viewportIndex].viewport = viewport;
        ...
        const onNewImage = (event) => {
            ...
            // index in the stack to the global JF.viewer.data object.
            ...
            if (stack && stack.data.length && stack.data[0].imageIds.length > 1) {
                ...
                JF.viewer.data.loadedSeriesData[viewportIndex].currentImageIdIndex = imageIdIndex;
            }
        ...
        JF.viewer.data.loadedSeriesData = layoutManager.viewportData;
        ...
        if (imagePlane && imagePlane.frameOfReferenceUID) {
            ...
            JF.viewer.updateImageSynchronizer.add(element);

            if (JF.viewer.refLinesEnabled) {
                // ... and if reference lines are globally enabled, let cornerstoneTools know.
                cornerstoneTools.referenceLines.tool.enable(element, JF.viewer.updateImageSynchronizer);
            }
...
Template.imageViewerViewport.onRendered(function() {
    ...
    function doubleClickHandler (event) {
        const $element = $(this);
        const { layoutManager } = JF.viewerbase;
...
Template.imageViewerViewport.onDestroyed(function() {
    ...
    JF.viewer.updateImageSynchronizer.remove(element);
