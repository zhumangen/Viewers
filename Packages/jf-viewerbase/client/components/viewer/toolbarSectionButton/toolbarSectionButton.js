...
Template.toolbarSectionButton.onCreated(() => {
    ...
    instance.isActive = activeToolId => {
        JF.managers.commands.last.dep.depend();
    ...
    instance.autorun(computation => {
        ...
        const lastCommand = JF.managers.commands.last.get();
        ...
        const activeToolId = JF.viewerbase.toolManager.getActiveTool();
...
Template.toolbarSectionButton.helpers({
    ...
    disableButton() {
        ...
        const instance = Template.instance();
        const isCommandDisabled = JF.managers.commands.isDisabled(instance.data.id);
...
Template.toolbarSectionButton.events({
    'click .toolbarSectionButton:not(.expandable)'(event, instance) {
        ...
        JF.managers.commands.run(instance.data.id);
    }
});
