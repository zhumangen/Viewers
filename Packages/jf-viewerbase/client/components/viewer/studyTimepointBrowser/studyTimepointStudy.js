...
Template.studyTimepointStudy.onCreated(() => {
    ...
    const studyMetadata = JF.viewerbase.getStudyMetadata(instance.data.study);
...
Template.studyTimepointStudy.events({
    ...
    'click .study-item-box'(event, instance) {
        ...
        if (!studyData.seriesList) {
            const alreadyLoaded = JF.viewer.Studies.findBy({ studyInstanceUid });

            if (!alreadyLoaded) {
                const $studies = instance.getStudyElement(true);
                $studies.trigger('loadStarted');
                JF.studies.retrieveStudyMetadata(studyInstanceUid).then(study => {
...
Template.studyTimepointStudy.helpers({
    isLoading() {
        ...
        const instance = Template.instance();
        const studyData = instance.data.study;
        const alreadyLoaded = JF.viewer.Studies.findBy({ studyInstanceUid: studyData.studyInstanceUid });
