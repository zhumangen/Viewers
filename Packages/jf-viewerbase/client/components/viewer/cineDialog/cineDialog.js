...
Template.cineDialog.onCreated(() => {
    ...
    instance.updateFramerate = rate => {
        JF.viewer.cine.framesPerSecond = rate;
        ...
        if (playClipData.intervalId !== void 0) {
            ...
            cornerstoneTools.playClip(element, JF.viewer.cine.framesPerSecond);
        } else {
            playClipData.framesPerSecond = JF.viewer.cine.framesPerSecond;
        }
    ...
    instance.api = {
        displaySetPrevious: () => JF.viewerbase.layoutManager.moveDisplaySets(false),
        displaySetNext: () => JF.viewerbase.layoutManager.moveDisplaySets(true),
        ...
        cineSlowDown: () => {
            const newValue = JF.viewer.cine.framesPerSecond - 1;
            if (newValue > 0) {
...
Template.cineDialog.events({
    ...
    'click .button-close'(event, instance) {
        JF.managers.commands.run('toggleCineDialog');
    }
});
...
Template.cineDialog.helpers({
    ...
    framerate() {
        ...
        return JF.viewer.cine.framesPerSecond.toFixed(1);
    },

    displaySetDisabled(isNext) {
        Session.get('LayoutManagerUpdated');

        // @TODO: Investigate why this is running while JF.viewerbase.layoutManager is undefined
        if (!JF.viewerbase.layoutManager) {
            return;
        }

        return !JF.viewerbase.layoutManager.canMoveDisplaySets(isNext) ? 'disabled' : '';
    },
