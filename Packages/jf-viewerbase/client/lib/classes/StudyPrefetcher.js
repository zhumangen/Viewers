...
export class StudyPrefetcher {
    ...
    prefetchActiveViewport() {
        const activeViewportElement = JF.viewerbase.viewportUtils.getActiveViewportElement();
    ...
    enablePrefetchOnElement(element) {
        ...
        if (this.hasStack(element)) {
            ...
            const displaySetInstanceUid = JF.viewer.data.loadedSeriesData[activeViewportIndex].displaySetInstanceUid;
    ...
    getStudy(image) {
        ...
        return JF.viewer.Studies.find(study => study.studyInstanceUid === studyMetadata.studyInstanceUid);
    }
