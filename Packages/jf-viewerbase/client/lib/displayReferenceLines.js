...
export function displayReferenceLines(element) {
    ...
    if (!JF.viewer.refLinesEnabled || !imagePlane || !imagePlane.frameOfReferenceUID) {
    ...
    $('.imageViewerViewport').not(element).each((index, viewportElement) => {
        ...
        if($(viewportElement).find('canvas').length) {
            ...
            cornerstoneTools.referenceLines.tool.enable(viewportElement, JF.viewer.updateImageSynchronizer);
        }
    });
}
