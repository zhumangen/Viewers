...
Template.hotkeysForm.onCreated(() => {
    ...
    instance.api = {
        save() {
            ...
            const promise = JF.managers.hotkeys.store(contextName, definitions);
            promise.then(() => {
                ...
                JF.managers.hotkeys.load(contextName).then(defs => instance.hotkeysDefinitions.set(defs));
            });
            return promise;
        },
        ...
        resetDefaults() {
            ...
            return OHIF.ui.showDialog('dialogConfirm', dialogOptions).then(() => {
                return JF.managers.hotkeys.resetDefauls(contextName);
            });
        }
    };
    ...
    const hotkeysContext = JF.managers.hotkeys.getContext(contextName) || {};
...
Template.hotkeysForm.events({
    ...
    'hotkeyChange .hotkey'(event, instance, data={}) {
        ...
        if (isModifier) {
            ...
            const commandsContext = JF.managers.commands.getContext(instance.data.contextName);
            const popoverData = {
...
Template.hotkeysForm.helpers({
    getHotkeyInputInformationLists() {
        JF.managers.hotkeys.changeObserver.depend();

        const instance = Template.instance();
        const { contextName } = instance.data;

        const hotkeyDefinitions = instance.hotkeysDefinitions.get();
        const commandsContext = JF.managers.commands.getContext(contextName);
        if (!hotkeyDefinitions || !commandsContext) return {};

        const commands = Object.keys(JF.managers.hotkeys.defaults[contextName] || {});
        const list = [];
