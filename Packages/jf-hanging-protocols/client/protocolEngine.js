...
const { OHIFError, metadata: { StudyMetadata, SeriesMetadata, InstanceMetadata, StudySummary } } = JF.viewerbase;
...
Meteor.startup(() => {
    HP.addCustomViewportSetting('wlPreset', 'Window/Level Preset', Object.create(null), (element, optionValue) => {
        if (_.findWhere(JF.managers.currentWLPresets, { id: optionValue })) {
            JF.managers.wlPresets.applyWLPreset(optionValue, element);
        }
    });
});
...
HP.ProtocolEngine = class ProtocolEngine {
    ...
    constructor(layoutManager, studies, priorStudies, studyMetadataSource) {
        ...
        const { LayoutManager, StudyMetadataSource } = JF.viewerbase;
        // -----------
        // Type Validations
        ...
        this.studies.forEach(study => {
            ...
            study.forEachSeries(series => {
                ...
                series.forEachInstance((instance, index) => {
                    ...
                    // rows = x00280010
                    if (!JF.viewerbase.isImage(instance.getTagValue('x00080016')) && !instance.getTagValue('x00280010')) {
                        return;
                    }
