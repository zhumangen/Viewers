...
JF.studies.retrieveStudyMetadata = (serverId, studyInstanceUid, seriesInstanceUids, sopInstanceUids) => {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        Meteor.call('GetStudyMetadata', serverId, studyInstanceUid, function(error, study) {
            ...
            JF.viewerbase.sortStudy(study);

            // Updates WADO-RS metaDataManager
            JF.viewerbase.updateMetaDataManager(study);

            // Transform the study in a StudyMetadata object
            const studyMetadata = new JF.metadata.StudyMetadata(study);

            // Add the display sets to the study
            study.displaySets = JF.viewerbase.sortingManager.getDisplaySets(studyMetadata);
