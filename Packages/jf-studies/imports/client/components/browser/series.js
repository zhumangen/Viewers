...
Template.studyBrowserSeries.onCreated(() => {
    ...
    instance.autorun(() => {
        ...
        if (!metadata) {
            metadata = JF.viewer.Studies.findBy({ studyInstanceUid });
        }
        ...
        if (!displaySets.length) {
            displaySets = JF.viewerbase.sortingManager.getDisplaySets(metadata);
            metadata.displaySets = displaySets;
            metadata.setDisplaySets(displaySets);

            metadata.forEachDisplaySet(displaySet => {
                JF.viewerbase.stackManager.makeAndAddStack(metadata, displaySet);
            });
        }
