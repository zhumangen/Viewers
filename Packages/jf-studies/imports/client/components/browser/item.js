...
Template.studyBrowserItem.onCreated(() => {
    ...
    if (JF.studies.getStudyBoxData) {
        JF.studies.getStudyBoxData(studyInformation).then(studyData => {
    ...
    instance.getStudyMetadata = () => {
        ...
        if (!instance.studyMetadata) {
            let study = JF.viewer.Studies.findBy({ studyInstanceUid }) || null;
            if (study && !(study instanceof JF.viewerbase.metadata.StudyMetadata)) {
                study = new JF.metadata.StudyMetadata(study, study.studyInstanceUid);
            }
    ...
    instance.autorun(() => {
        ...
        JF.studies.loadingDict.get(studyInstanceUid);
