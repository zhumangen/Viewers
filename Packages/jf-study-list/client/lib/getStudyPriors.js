...
const getStudyPriors = study => {
    ...
    if (!(study instanceof StudyMetadata) && !(study instanceof StudySummary)) {
        throw new JF.viewerbase.OHIFError('getStudyPriors study must be an instance of StudySummary or StudyMetadata');
    }
    ...
    const cursor = JF.studylist.collections.Studies.find({
        patientId: patientID,
        studyDate: {
            $lt: studyDate
        }
    ...
    // Create an JFStudySummary object for each prior study found
    cursor.forEach(study => {
        const summary = new JF.studies.classes.OHIFStudySummary(study, null, study.studyInstanceUid);
